        ...
        <div class="wrapper_full_1">
            <div class="wrapper_full_4">
                <div class="title_cate_list"> <a href="{{ route('site.home') }}" class="fist_menu_cate_a">Trang chủ</a>
                    <a href="{{ route('site.showAllProduct') }}">Tất cả sản phẩm</a>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <ul class="ul_list_recieve_text">
                            ...
                            @foreach ($parentCategories as $parentCategory)
                                <li class="c_parent"><a
                                        ...
                                        title="{{ $parentCategory->name }}">{{ $parentCategory->name }}<b>({{ count($parentCategory->products) }})</b></a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                            ...
                            <ul class="ul_list_recieve_text_1" id="panel_filter_price" data-label="Giá">
                                ...
                                <li>
                                    <label for="filter-800-1200">
                                        <input type="radio" id="filter-800-1200" name="filter-price"
                                        ...
                                        @foreach ($brands as $brand)
                                            <li>
                                                ...
                                                <label for="filter-{{ $brand->id }}">
                                                    <input type="radio" id="filter-{{ $brand->id }}"
                                                        name="filter-brand" value="{{ $brand->id }}"
                                                        data-url="{{ request()->fullUrlWithQuery(['brand_id' => $brand->id]) }}"
                ...
                <div class="right_list_product">
                    <div class="box_cate_add">
                        ...
                        <div class="box_cate_news">
                            <ul>
                                @foreach ($parentCategories as $parentCategory)
                                    <li><a href="{{ route('site.parentCategory.index', [$parentCategory->slug]) }}"
                                            title="{{ $parentCategory->name }}">{{ $parentCategory->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                                            ...
                                            <ul class="pagination" role="navigation">
                                                @if ($page > 1)
                                                    <li class="page-item"> <a class="page-link"
                                                            ...
                                                            rel="next" aria-label="&laquo; Previous">&lsaquo;</a>
                                                    </li>
                                                @endif
