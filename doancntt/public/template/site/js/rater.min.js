(function ($, window) {
    ...
    Rate.prototype.hover = function (ev) {
        var pad = parseInt(
            $(this.element).css("padding-left").replace("px", "")
        );
    ...
    Rate.prototype.toValue = function (width, in_pixels) {
        ...
        if (in_pixels) {
            val =
                (width / this.layers.base_layer.textWidth()) *
        ...
        if (temp - Math.floor(temp) < 5e-5) {
            val =
                Math.round(val / this.settings.step_size) *
                this.settings.step_size;
        }
        val =
            Math.ceil(val / this.settings.step_size) * this.settings.step_size;
    ...
    Rate.prototype.setValue = function (value) {
        if (!this.settings.readonly) {
            ...
            if (this.set_faces.hasOwnProperty(index_value)) {
                ...
                var base_layer_element = this.getElement(
                    "base-layer",
                    index_value
                );
                var select_layer_element = this.getElement(
                    "select-layer",
                    index_value
                );
                var hover_layer_element = this.getElement(
                    "hover-layer",
                    index_value
                );
                var left_pos =
