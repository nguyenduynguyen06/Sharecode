...
Route::group(['middleware' => ['permission:Xem mô tả sản phẩm']], function () {
    Route::get('/mo-ta/{product}', [ProductController::class, 'description'])->name('description');
    Route::put('/mo-ta/{product}', [ProductController::class, 'updateDescription'])->name('updateDescription');
});

Route::group(['middleware' => ['permission:Xem mô tả chi tiết sản phẩm']], function () {
    Route::get('/mo-ta-chi-tiet/{product}', [ProductController::class, 'descriptionDetails'])->name('descriptionDetails');
    Route::put('/mo-ta-chi-tiet/{product}', [ProductController::class, 'updateDescriptionDetails'])->name('updateDescriptionDetails');
});

Route::group(['middleware' => ['permission:Xem hướng dẫn sử dụng sản phẩm']], function () {
    Route::get('/huong-dan-su-dung/{product}', [ProductController::class, 'useTutorials'])->name('useTutorials');
    Route::put('/huong-dan-su-dung/{product}', [ProductController::class, 'updateUseTutorials'])->name('updateUseTutorials');
});
